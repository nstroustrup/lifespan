        ...
        for ( j = pad_left ? 0 : r; j < (pad_right ? n : n-r); ++j ) {
            for ( c = 0; c < cn; ++c ) {
                ...
                for ( b = 0; b < 16 ; ++b ) {
                    ...
                    if ( sum > t ) {
						#define MED 16*k + b
						#define VAL src[src_step*i + cn*j+c] 

						dst[dst_step*i + cn*j+c] = (VAL<MED)*(MED-VAL);
						//if (VAL == 0)
						//	dst[dst_step*i + cn*j+c] = 0;
						//else dst[dst_step*i + cn*j+c] = ((255-VAL)/(255-MED));
						#undef MED
						#undef VAL
        ...
        )
{
    ...
    for ( i = 0; i < width; i += stripe_size - 2*r ) {
        ...
        ctmf_helper_orig( src + cn*i, dst + cn*i, stripe, height, src_step, dst_step, r, cn,
                i == 0, stripe == width - i );
