...
#prevent oversubscription of directories, e.g. more array job subdivisions reqested than folders
if [ $num_tasks -gt $num_dirs ]
then
	num_tasks=$num_dirs;
fi

#we must have only one process per folder, so stop if we're not needed
if [ $num_tasks -lt $jid ]
then
	exit
fi
