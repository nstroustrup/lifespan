...
public:
 ns_ojp2k_image_input_file() :lines_read(0),low_memory_single_line_reads(false), data(0), lines_sent(0), total_lines_sent(0),input_buffer(0), input_buffer_size(0)
...
public:
	...
	void open_file(const std::string & filename, const ns_image_properties & properties) {
	  const float compression_ratio = .05;
	  if (data != 0)
