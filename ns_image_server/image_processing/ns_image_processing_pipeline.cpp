...
void ns_lifespan_curve_cache_entry::load_from_external_source(const ns_annotation_region_data_id & id, ns_sql & sql) {
	...
	for (unsigned int i = 0; i < res.size(); i++) {
		...
		if (reload_experiment_data) {
			...
			set.generate_aggregate_risk_timeseries(p->second.metadata, true, 0, p->second.movement_survival, p->second.death_associated_expansion_survival, p->second.risk_timeseries_time);
