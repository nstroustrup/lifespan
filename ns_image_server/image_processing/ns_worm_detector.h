...
public:
	ns_image_worm_detection_results * run(const unsigned long region_info_id,const unsigned long capture_time,const whole_image & unprocessed_image,const whole_image & thresholded_image, const whole_image & spatial_median_image, const ns_image_standard * static_mask, const unsigned int & minimum_region_area, const unsigned int &maximum_region_area, const unsigned int &maximum_region_diagonal, const ns_svm_model_specification & model, const unsigned long maximum_number_of_objects, const std::string & spine_debug_filename_output="", const ns_detected_worm_info::ns_visualization_type visualization_type=ns_detected_worm_info::ns_vis_raster,ns_whole_image_region_stats image_region_stats = ns_whole_image_region_stats::null(),const bool do_multiple_worm_disambiguation=true){
		...
		std::cerr << object_manager.objects.size() << "/" << init_object_count << " objects fall within size limits.\n";
		...
		ns_image_worm_detection_results * tmp(run(region_info_id,capture_time,object_manager,unprocessed_image,spatial_median_image,static_mask,model,maximum_number_of_objects,spine_debug_filename_output,visualization_type,image_region_stats));
	...
	ns_image_worm_detection_results * run(const unsigned long region_info_id,const unsigned long capture_time,ns_detected_object_manager & detected_objects, const whole_im & unprocessed_image,const whole_im & spatial_median_image, const ns_image_standard * static_mask, const ns_svm_model_specification & model, const unsigned long maximum_number_of_objects,const std::string & debug_filename_output="", const ns_detected_worm_info::ns_visualization_type visualization_type=ns_detected_worm_info::ns_vis_raster, ns_whole_image_region_stats whole_image_region_stats = ns_whole_image_region_stats::null(),const bool do_multiple_worm_disambiguation=true){
		const unsigned long start_time = ns_current_time();
		const unsigned long s(detected_objects.objects.size());
		hande_static_mask(detected_objects,spatial_median_image, static_mask);
		std::cerr << s-detected_objects.objects.size() << " objects were removed by the static mask.\n";
	...
	//		cerr << "Generating region results\n";
			...
			results->process_segment_cluster_solutions(detected_objects.objects,spatial_median_image,unprocessed_image,visualization_type,ns_worm_detection_constants::get(ns_worm_detection_constant::maximum_number_of_putative_worms_per_image));
			...
			else std::cerr << results->number_of_actual_worms() << " worms were detected\n";
