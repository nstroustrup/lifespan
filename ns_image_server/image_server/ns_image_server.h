...
public:
	...
	///image server nodes check the cluster to see if they are running the latest version of the software,
	///which is specified by (major).(minor).(software_version_compile).
	unsigned int software_version_minor(){return 6;}
	///image server nodes check the cluster to see if they are running the latest version of the software,
	///which is specified by (major).(minor).(software_version_compile).
	...
	std::string create_and_configure_sql_database(bool local, const std::string & schema_specification_filename="");
	...
	ns_64_bit register_server_event(const ns_ex & s_event, ns_image_server_sql * sql, const bool no_display = false)const;
