...
public:
	ns_image_storage_reciever_to_disk(const unsigned long max_block_height, const std::string & filename, const ns_image_type & image_type, const float compression_ratio,const bool volatile_file_ = false, const ns_dir::ns_output_file_permissions perm = ns_dir::ns_output_file_permissions::ns_no_special_permissions) :
		...
		file_sink(filename, ns_choose_image_source<ns_image_output_file<ns_component> >(image_type,jpeg_out, tiff_out, jp2k_out), max_block_height, compression_ratio), tiff_out(ns_get_tiff_compression_type(image_type))
	{
		...
		if (perm!= ns_dir::ns_output_file_permissions::ns_no_special_permissions)
