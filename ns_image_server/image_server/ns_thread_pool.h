...
public:
	...
	void set_number_of_threads(unsigned long i) {
		...
		threads.clear();
		threads.resize(i);
		for (unsigned int j = 0; j < thread_idle_locks.size(); j++)
			delete thread_idle_locks[j];
		thread_idle_locks.clear();
		thread_idle_locks.resize(i);
		for (unsigned int j = 0; j < i; j++)
			thread_idle_locks[j] = new ns_lock(std::string("ns-tpi-") + ns_to_string(j));
	}
	...
	//important!  This /keeps/ the job_access_lock and the wait_after_jobs_finish lock
	...
	void shutdown() {
		...
		//this will release the hold and start the queue running again
	//	if (state == ns_holding_idle_threads)
	//		release_hold_on_idle_threads();

		//this will wait for the polling process to end, and the queue to enter a paused state.
		...
		//if the queue is paused, we need to get it to run so that each thread can run its termination routine.
		if (state == ns_paused) 
			run_pool();
		...
		void prepare_pool_to_run() {
			...
			for (unsigned int i = 0; i < threads.size(); i++) {
				...
				//std::cerr << "Sending: " << reinterpret_cast<ns_64_bit>(data->pool) << "\n";
				threads[i].run(this->run, data);
			}
			state = ns_paused;
		}
		void run_pool() {
			//std::cerr << "Me: " << reinterpret_cast<ns_64_bit>(this) << "\n";
			if (state != ns_paused)
			...
			bool all_threads_initialized(false);
			while (true) {

				NS_TPDBG(debug << "(main waiting for thread init count...)");
				thread_init_lock.wait_to_acquire(__FILE__, __LINE__);
				all_threads_initialized = thread_init_count >= threads.size();
				thread_init_lock.release();
				if (all_threads_initialized || shutdown_)
					break;
				ns_thread::sleep_milliseconds(1);
			}
			job_access_lock.release();
		...
		//	std::cerr << "rem:" << reinterpret_cast<ns_64_bit>(p.pool) << "\n";
			if (p.thread_id >= p.pool->thread_idle_locks.size())
				throw ns_ex("Invalid thread id!");
			...
			while (true) {
				...
				if (p.pool->jobs.empty()) {
					...
					if (p.pool->shutdown_) //if shutdown is called while the pool state is ns_holding_idle_threads
						return 0;
