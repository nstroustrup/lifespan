...
void ns_time_path_image_movement_analyzer::save_movement_data_to_db(const ns_64_bit region_id, ns_sql & sql){
...
void ns_time_path_image_movement_analyzer::load_movement_data_from_disk(istream & in, bool skip_movement_data){
	...
	//	cerr << "[" << groups[group_id].paths[path_id].elements[element_id].measurements.interframe_time_scaled_movement_sum << "]";
		...
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_movement_sum);//18
		if(in.fail()) throw ns_ex("Invalid Specification");
		char a(get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_stationary_sum));//19
		if(in.fail()) throw ns_ex("Invalid Specification");
		if (a == '\n'){
			//old style records
		...
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_position.x);//20
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_position.y);//21
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_area.x);//22
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.local_maximum_area.y);//23
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.unnormalized_movement_sum);//24
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_double(in,groups[group_id].paths[path_id].elements[element_id].measurements.denoised_movement_score);//25
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_double(in,groups[group_id].paths[path_id].elements[element_id].measurements.movement_score);//26
		if(in.fail()) throw ns_ex("Invalid Specification");
		get_int(in,groups[group_id].paths[path_id].elements[element_id].measurements.change_in_total_worm_intensity);//27
		if(in.fail()) throw ns_ex("Invalid Specification");
		...
		for (unsigned long j = 0; j < groups[i].paths.size(); j++){
			for (unsigned long k = 0; k < groups[i].paths[j].elements.size(); k++){
				o << i << ","//1
					...
					<< groups[i].paths[j].elements[k].measurements.local_maximum_movement_sum << ","//18
					<< groups[i].paths[j].elements[k].measurements.local_maximum_stationary_sum << ","//19
					<< groups[i].paths[j].elements[k].measurements.local_maximum_position.x << ","//20
					<< groups[i].paths[j].elements[k].measurements.local_maximum_position.y << ","//21
					<< groups[i].paths[j].elements[k].measurements.local_maximum_area.x << ","//22
					<< groups[i].paths[j].elements[k].measurements.local_maximum_area.y << ","//23
...
void ns_analyzed_image_time_path::generate_movement_images(const ns_analyzed_time_image_chunk & chunk){
	...
	for (long i = chunk.start_i; ; i += step) {
		...
		flow->Dim1.init(elements[i].registered_images->movement_image_.properties());
		flow->Dim2.init(elements[i].registered_images->movement_image_.properties());
		float * dd1(flow->Dim1.buffer());
		float * dd2(flow->Dim2.buffer());
		...
		for (unsigned long y = 0; y < prop.height; y++)
			for (unsigned long x = 0; x < prop.width; x++){
				temp_im[y][3*x+0]=d1[flow_w*y + x ];
				temp_im[y][3*x+1]=d2[flow_w*y + x ];
				temp_im[y][3*x+2] = 0;
			}
...
void ns_analyzed_image_time_path::save_movement_images(const ns_analyzed_time_image_chunk & chunk, ns_sql & sql, const bool save_image, const bool save_flow_image) {
	//handle small or non-existant images
	ns_movement_image_collage_info d(this/*,chunk.stop_i-chunk.start_i*/);
	if (d.prop.width == 0 || d.prop.height == 0) {
		...
		if (save_image) {
			output_reciever = new ns_image_storage_reciever_handle<ns_8_bit>(image_server.image_storage.request_storage(output_image, ns_tiff_lzw, 1024, &sql, had_to_use_volatile_storage, false, false));
			im.pump(output_reciever->output_stream(), 1024);
		}
		if (save_flow_image) {
			flow_output_reciever = new ns_image_storage_reciever_handle<float>(image_server.image_storage.request_storage_float(flow_output_image, ns_tiff_uncompressed, 1024, &sql, had_to_use_volatile_storage, false, false));
	...
	if (save_image ) 
		save_movement_image(chunk, *output_reciever);
	if (save_flow_image)
		save_movement_flow_image(chunk, *flow_output_reciever);
...
void ns_analyzed_image_time_path::save_movement_image(const ns_analyzed_time_image_chunk & chunk, ns_image_storage_reciever_handle<ns_8_bit> & out) {
...
void ns_analyzed_image_time_path::save_movement_flow_image(const ns_analyzed_time_image_chunk & chunk, ns_image_storage_reciever_handle<float> & flow_out){
...
void ns_analyzed_image_time_path::initialize_movement_image_loading(ns_image_storage_source_handle<ns_8_bit> & in,ns_image_storage_source_handle<float> & flow_in){
	if (elements.size() == 0) return;
	movement_loading_collage_info.from_path(this);
...
bool ns_time_path_image_movement_analyzer::load_movement_image_db_info(const ns_64_bit region_id,ns_sql & sql){
	...
	for (unsigned int i = 0; i < res.size(); i++){
		...
		if (group_id >= groups.size() || (!groups[group_id].paths.empty() && path_id >= groups[group_id].paths.size())){
			...
			sql.send_query();
			image_server.register_server_event(ns_ex("ns_time_path_image_movement_analyzer::load_movement_image_db_info()::Deleting path data as a too-large group or path_id was found: (") << group_id << "," << path_id << ") in a region with " << groups.size() << " groups",&sql);
		}
		else{
...
void ns_time_path_image_movement_analyzer::load_images_for_group(const unsigned long group_id, unsigned long number_of_images_to_load,ns_sql & sql, const bool load_images_after_last_valid_sample){
	...
	for (unsigned int j = 0; j < groups[group_id].paths.size(); j++){
		...
		if (number_of_images_loaded == 0){
			...
			groups[group_id].paths[j].initialize_movement_image_loading(groups[group_id].paths[j].movement_image_storage,groups[group_id].paths[j].flow_movement_image_storage);
		}
	...
	try{
		...
		for (unsigned int i = 0; i < groups.size(); i++){
			...
			for (unsigned int j = 0; j < groups[i].paths.size(); j++){
				...
				if (calculate_flow_images) {
							bool had_to_use_volatile_storage;
							groups[i].paths[j].flow_output_reciever = new ns_image_storage_reciever_handle<float>(image_server.image_storage.request_storage_float(groups[i].paths[j].flow_output_image, ns_tiff_uncompressed, 1024, &sql, had_to_use_volatile_storage, false, false));
					...
					groups[i].paths[j].initialize_movement_image_loading(storage, flow_storage);
				}
				...
				for (unsigned int k = 0; k < number_of_valid_elements; k+=chunk_size){
					...
					if (calculate_flow_images) {
						...
						groups[i].paths[j].save_movement_images(chunk, sql,false,true);
					}
					else {
