...
ns_vector_2d ns_analyzed_image_time_path_element::worm_center_in_registered_image() const {
	if (worm_center_in_registered_image_cached_calculated)
	...
	const long h(registered_images->worm_region_threshold.properties().height),
			w(registered_images->worm_region_threshold.properties().width);
	for (unsigned int y = 0; y < h; y++)
		for (unsigned int x = 0; x < w; x++){
			if (registered_images->get_worm_threshold(y, x)) {
				px += x;
				py += y;
				n++;
			}
...
ns_vector_2d ns_analyzed_image_time_path_element::worm_center_in_absolute_coordinates() const {
	ns_vector_2d t(worm_center_in_registered_image());
	if (t.x == -1)
		return t;
	return t - registration_offset;
}
...
struct ns_movement_analysis_shared_state {
	ns_movement_analysis_shared_state():open_object_count(0),sql_lock("ssl"), deallocated_aligned_count(0),
		registered_image_clear_lag(std::max(ns_analyzed_image_time_path::movement_time_kernel_width, ns_analyzed_image_time_path::intensity_normalization_kernel_half_width)),
		sql(0),
		path_aligned_image_clear_lag(std::max(std::max(ns_analyzed_image_time_path::movement_time_kernel_width, ns_analyzed_image_time_path::alignment_time_kernel_width), ns_analyzed_image_time_path::intensity_normalization_kernel_half_width)){}
...
void ns_time_path_image_movement_analyzer<allocator_T>::run_group_for_current_backwards_round(unsigned int group_id, unsigned int path_id, ns_time_path_image_movement_analyzer_thread_pool_persistant_data * persistant_data,ns_movement_analysis_shared_state * shared_state) {
	...
	ns_analyzed_time_image_chunk movement_analysis_chunk(chunk);
	movement_analysis_chunk.start_i += shared_state->registered_image_clear_lag;
	movement_analysis_chunk.stop_i += shared_state->registered_image_clear_lag;

	if (movement_analysis_chunk.start_i <= groups[i].paths[j].first_stationary_timepoint() - 1) {
		groups[i].paths[j].calculate_movement_images(movement_analysis_chunk, shared_state->chunk_generators[i][j].first_chunk());
...
#else
		groups[i].paths[j].save_movement_images(movement_analysis_chunk, *shared_state->sql, ns_analyzed_image_time_path::ns_save_simple, ns_analyzed_image_time_path::ns_only_output_backwards_images,ns_analyzed_image_time_path::ns_local_0);
#endif
		sql_lock.release();
	}
	...
	if (shared_state->chunk_generators[i][j].no_more_chunks_backward()) {
		...
		ns_analyzed_time_image_chunk movement_analysis_chunk;
		movement_analysis_chunk.processing_direction = ns_analyzed_time_image_chunk::ns_backward;
		movement_analysis_chunk.start_i = shared_state->registered_image_clear_lag - 1;
		movement_analysis_chunk.stop_i = -1;
		groups[i].paths[j].calculate_movement_images(movement_analysis_chunk, shared_state->chunk_generators[i][j].first_chunk());
		ns_acquire_lock_for_scope sql_lock(shared_state->sql_lock, __FILE__, __LINE__);
#ifdef NS_CALCULATE_OPTICAL_FLOW
		groups[i].paths[j].save_movement_images(registration_chunk, *shared_state->sql, ns_analyzed_image_time_path::ns_save_both, ns_analyzed_image_time_path::ns_only_output_backwards_images, ns_analyzed_image_time_path::ns_local_0);
...
void ns_time_path_image_movement_analyzer<allocator_T>::finish_up_and_write_to_long_term_storage(unsigned int group_id, unsigned int path_id, ns_time_path_image_movement_analyzer_thread_pool_persistant_data * persistant_data, ns_movement_analysis_shared_state * shared_state) {
	for (unsigned int i = 0; i < groups[group_id].paths[path_id].element_count(); i++) {
		if (groups[group_id].paths[path_id].elements[i].debug_write_count != 1)
			cout << "G" << group_id << ":" << groups[group_id].paths[path_id].elements[i].debug_write_count << " (" << groups[group_id].paths[path_id].first_stationary_timepoint() << "," << groups[group_id].paths[path_id].element_count() << ")\n";
		groups[group_id].paths[path_id].elements[i].debug_write_count = 0;
	}
...
void ns_time_path_image_movement_analyzer<allocator_T>::run_group_for_current_forwards_round(unsigned int group_id, unsigned int path_id, ns_time_path_image_movement_analyzer_thread_pool_persistant_data * persistant_data,ns_movement_analysis_shared_state * shared_state) {
	const unsigned int &g(group_id), &p(path_id);
	ns_analyzed_image_time_path& path(groups[g].paths[p]);
	...
	ns_analyzed_time_image_chunk chunk;
	if (!shared_state->chunk_generators[g][p].update_and_check_for_new_chunk(chunk))
		return;
	...
	//we need data from the "future" to do the intensity normalization
	//so we need to be one behind in the movement calculation at all times.
	ns_analyzed_time_image_chunk movement_analysis_chunk(chunk);
	movement_analysis_chunk.start_i -= shared_state->registered_image_clear_lag;
	if (!shared_state->chunk_generators[g][p].no_more_chunks_forward())
		movement_analysis_chunk.stop_i -= shared_state->registered_image_clear_lag;
	if (movement_analysis_chunk.start_i < shared_state->chunk_generators[g][p].first_chunk().start_i)
		movement_analysis_chunk.start_i = shared_state->chunk_generators[g][p].first_chunk().start_i;
		groups[g].paths[p].calculate_movement_images(movement_analysis_chunk, shared_state->chunk_generators[g][p].first_chunk());

#ifdef NS_OUTPUT_ALGINMENT_DEBUG
		for (unsigned int k = chunk.start_i; k < chunk.stop_i; k++) {
			oout << k << "," << groups[p].paths[g].element(k).registration_offset.x << "," <<
				path.element(k).registration_offset.y << "," <<
				path.element(k).registration_offset.mag() << "\n";
		}
		oout.flush();
#endif
		...
		if (movement_analysis_chunk.start_i != movement_analysis_chunk.stop_i){
			ns_acquire_lock_for_scope sql_lock(shared_state->sql_lock, __FILE__, __LINE__);
#ifdef NS_CALCULATE_OPTICAL_FLOW
			path.save_movement_images(chunk, *shared_state->sql, ns_analyzed_image_time_path::ns_save_both, ns_analyzed_image_time_path::ns_output_all_images, ns_analyzed_image_time_path::ns_local_1);
#else
			path.save_movement_images(movement_analysis_chunk, *shared_state->sql, ns_analyzed_image_time_path::ns_save_simple, ns_analyzed_image_time_path::ns_output_all_images, ns_analyzed_image_time_path::ns_local_1);
#endif
			sql_lock.release();
		}
	...
	for (long k = 0; k < (long)movement_analysis_chunk.start_i - (long)shared_state->registered_image_clear_lag ; k++)
		path.elements[k].clear_movement_images(memory_pool.registered_image_pool);

	if (shared_state->chunk_generators[g][p].no_more_chunks_forward()) {


		for (long k = 0; k < path.elements.size(); k++) {
			path.elements[k].clear_path_aligned_images(memory_pool.aligned_image_pool);
			path.elements[k].clear_movement_images(memory_pool.registered_image_pool);
		}
		path.reset_movement_image_saving();
	}
}
template<class allocator_T>
...
void ns_time_path_image_movement_analyzer<allocator_T>::process_raw_images(const ns_64_bit region_id,const ns_time_path_solution & solution_, const ns_time_series_denoising_parameters & times_series_denoising_parameters,
	...
	try {
		...
		get_output_image_storage_locations(region_id, group_number, sql, false);
		...
		for (unsigned int i = 0; i < groups.size(); i++) {
			if (group_number != -1 && i != group_number)
				continue;
		...
		if (number_of_paths_to_consider == 0)
			...
			const ns_64_bit number_of_repeats_required((group_number != -1) ? 1 :
			...
			const int number_of_worms_per_repeat((int)((group_number != -1) ? 1 : ceil(number_of_paths_to_consider / (float)number_of_repeats_required)));
			...
			int current_round(0);	//otherwise, we may need to split a large number of worms into multiple rounds, so we figure out if that's necissary.

			//go through all the groups, processing "number_of_worms_per_repeat" groups per iteration
			for (unsigned int g = 0; g < groups.size(); ) {
				const unsigned long start_group(g);
				unsigned long stop_group;

				if (group_number != -1) {	// if we are explicitly targeting only one worm, just process that
					if (g < group_number) {
						g = group_number;
						continue;
					}
				...
				//it's tricky to handle all the logic of which images need to be newly loaded in each step
				//so we just keep a list and compare it to what's needed in each step, and load the difference.
				std::vector<char> region_vis_images_loaded(region_image_specifications.size(), 0);
				int last_r = 0;
				for (long t = (long)region_image_specifications.size() - 1; t >= 0; t--) {
					...
					bool found_image_to_process = false;
					try {
						...
						long first_image_to_load = stop_t + 1,
						...
						for (unsigned int imm = first_image_to_load; imm <= last_image_to_load && !found_image_to_process; imm++) {
							for (unsigned int gg = start_group; gg < stop_group && !found_image_to_process; gg++) {
								for (unsigned int pp = 0; pp <  groups[gg].paths.size() && !found_image_to_process; pp++) {
									for (unsigned int tt = 0; tt < groups[gg].paths[pp].element_count() && !found_image_to_process; tt++) {
										if (groups[gg].paths[pp].element(tt).absolute_time == region_image_specifications[imm].time) {
											found_image_to_process = true;
											break;
										}
										if (groups[gg].paths[pp].element(tt).absolute_time > region_image_specifications[imm].time)
											break;
						...
						if (found_image_to_process) {
							//std::cerr << "spec " << t << ": chunk size:" << chunk_size << " Requesting Backwards: " << first_image_to_load << " to " << last_image_to_load + 1 << "\n";
							load_region_visualization_images(first_image_to_load, last_image_to_load + 1, start_group, stop_group, sql, false, true, ns_analyzed_image_time_path::ns_lrv_flag_and_images, group_number);
						}
						for (long t1 = first_image_to_load; t1 <= last_image_to_load; t1++)
							region_vis_images_loaded[t1] = 1;
						...
						try {
							load_region_visualization_images(0, region_image_specifications.size(), 0, groups.size(), sql, true, true, ns_analyzed_image_time_path::ns_lrv_flag_and_images, group_number);
						}
						catch (ns_ex & ex2) {
							ex_f << ";" << ex2.text();
						}
				...
				for (unsigned int i = start_group; i < stop_group; i++) {
					for (unsigned int j = 0; j < groups[i].paths.size(); j++) {
						ns_analyzed_image_time_path& p(groups[i].paths[j]);
						for (int e = 0; e < p.first_stationary_timepoint(); e++) {
							if (p.elements[e].debug_write_count != 1)
								cout << "Image not written: " << i << "::" << e << "\n";
							p.elements[e].debug_write_count = 0;
						}
	...
	//					groups[i].paths[j].reset_movement_image_saving();
			...
			//		shared_state.path_reset[i] = 0;
				...
				for (unsigned int t = 0; t < region_image_specifications.size(); t += chunk_size) {
					...
					bool found_image_to_process = false;
					for (unsigned int imm = t; imm < image_load_stop_t && !found_image_to_process; imm++) {
						for (unsigned int gg = start_group; gg < stop_group && !found_image_to_process; gg++) {
							for (unsigned int pp = 0; pp < groups[gg].paths.size() && !found_image_to_process; pp++) {
								for (unsigned int tt = 0; tt < groups[gg].paths[pp].element_count() && !found_image_to_process; tt++) {
									if (groups[gg].paths[pp].element(tt).absolute_time == region_image_specifications[imm].time) {
					...
					if (found_image_to_process) {
						try {
							...
							//std::cerr << "Requesting forwards: " << image_load_start_t << " to " << image_load_stop_t << "\n";
							load_region_visualization_images(image_load_start_t, image_load_stop_t, start_group, stop_group, sql, false, false, ns_analyzed_image_time_path::ns_lrv_flag_and_images, group_number);
							for (long tt = image_load_start_t; tt < image_load_stop_t; tt++)
								region_vis_images_loaded[tt] = 1;
						}
						catch (ns_ex & ex) {
							ns_ex ex_f(ex);
							...
							try {
								load_region_visualization_images(0, region_image_specifications.size(), 0, groups.size(), sql, true, false, ns_analyzed_image_time_path::ns_lrv_flag_and_images, group_number);
							}
							catch (ns_ex & ex2) {
								ex_f << ";" << ex2.text();
							}
		...
		if (number_of_paths_to_consider > 0) {
			...
			if (groups.size() > 0 && write_status_to_db && group_number != -1) {
				int last_p(0);
				while (true) {
...
template<class allocator_T>
void ns_time_path_image_movement_analyzer<allocator_T>::get_output_image_storage_locations(const ns_64_bit region_id, const ns_64_bit& specific_worm_id, ns_sql & sql,const bool create_only_flow){
	...
	for (unsigned int i = 0; i < groups.size(); i++){
		for (unsigned int j = 0; j < groups[i].paths.size(); j++){
			if (specific_worm_id != -1 && i != specific_worm_id)
				continue;
			if (ns_skip_low_density_paths && groups[i].paths[j].is_low_density_path())
				continue;
...
void ns_analyzed_image_time_path_element_measurements::read(istream & in, ns_vector_2d & registration_offset, ns_vector_2d& intensity_center_of_mass,bool & saturated_offset)  {
	...
	if (tmp == "")
		intensity_center_of_mass.y = 0;
	else intensity_center_of_mass.y = atof(tmp.c_str());
}
template<class allocator_T>
void ns_time_path_image_movement_analyzer<allocator_T>::load_movement_data_from_disk(istream & in, bool skip_movement_data){
	...
	while(true){
		...
		groups[group_id].paths[path_id].elements[element_id].histogram_matching_data.overall_scaling = groups[group_id].paths[path_id].elements[element_id].measurements.intensity_normalization_scale_factor;
		//std::cout << "\n";
...
void ns_analyzed_image_time_path_element_measurements::write(ostream & out,const ns_vector_2d & offset, const ns_vector_2d & intensity_center_of_mass,const bool & saturated_offset) const {
	out << spatial_averaged_movement_sum_cropped[0] << ","//5
		...
		raw_flow_dy.write(o);
		#endif
		<< intensity_center_of_mass.x << ","
		<< intensity_center_of_mass.y;
}
template<class allocator_T>
	...
	for (unsigned long i = 0; i < groups.size(); i++){
		for (unsigned long j = 0; j < groups[i].paths.size(); j++){
			for (unsigned long k = 0; k < groups[i].paths[j].elements.size(); k++){
				...
				groups[i].paths[j].elements[k].measurements.write(o,groups[i].paths[j].elements[k].registration_offset, groups[i].paths[j].elements[k].intensity_center_of_mass, groups[i].paths[j].elements[k].saturated_offset);
				o <<"\n";
			}
		}
	}
...
std::string ns_calc_rel_exit_time_by_index(const unsigned long time, const ns_movement_state_observation_boundary_interval& i, const ns_analyzed_image_time_path& path) {
	if (i.skipped)
		return "";
	return ns_to_string_short((((long)time) - (long)path.state_exit_interval_time(i).best_estimate_event_time_for_possible_partially_unbounded_interval()) / (60.0 * 60 * 24), 3);
}
...
void ns_analyzed_image_time_path::write_detailed_movement_quantification_analysis_data(const ns_region_metadata & m, const unsigned long group_id, const unsigned long path_id, const ns_death_time_annotation_time_interval& first_plate_observation_interval, std::ostream & o, const bool output_only_elements_with_hand,const bool abbreviated_time_series)const{
	...
	for (unsigned long k = start_index; k < end_index; k++){
		...
		// "Registration Offset X, Registration Offset Y, Registration Offset Magnitude,"
		// "Absolute Time, Age Relative Time,"
			<< elements[k].registration_offset.x << ","
			<< elements[k].registration_offset.y << ","
			<< elements[k].registration_offset.mag() << ","
...
void ns_analyzed_image_time_path::detect_death_times_and_generate_annotations_from_movement_quantification(const ns_stationary_path_id & path_id, const ns_analyzed_image_time_path_death_time_estimator * movement_death_time_estimator, ns_movement_analysis_result & analysis_result, const ns_death_time_annotation_time_interval& first_plate_observation_interval, const unsigned long last_time_point_in_analysis, ns_analyzed_image_time_path_death_time_estimator_reusable_memory& mem) const {
	...
	analysis_result.death_time_annotation_set.add(
		ns_death_time_annotation(ns_first_observation_on_plate,
			0, region_info_id,
			first_plate_observation_interval,
			ns_vector_2i(0,0),  //register the position of the object at that time point
			ns_vector_2i(0,0),
			exclusion_type,
...
ns_vector_2i ns_image_intensity_center_from_channel(const ns_image_standard& im, const ns_vector_2i& tl, const ns_vector_2i& br, const char channel) {
	double sx(0), sy(0), is(0);
	//ns_vector_2i maxx(0, 0);
	//double maxv(0);
	for (unsigned long y = tl.y; y < br.y; y++) {
		for (unsigned long x = tl.x; x < br.x; x++) {
			const double v(im[y][3 * (x) + channel] / 255.0);
			sx += (x - tl.x) * v;
			sy += (y - tl.y) * v;
			is += v;
		}
...
ns_vector_2i ns_image_intensity_center(const ns_image_whole<double>& im, const ns_image_whole<ns_16_bit> & count,const ns_vector_2i& tl, const ns_vector_2i& br) {
	double sx(0), sy(0), is(0);
	for (unsigned long y = tl.y; y < br.y; y++) {
		for (unsigned long x = tl.x; x < br.x; x++) {
			const double v((count[y][x]!=0)?im[y][x ]/count[y][x] : 0);
			sx += (x - tl.x) * v;
			sy += (y - tl.y) * v;
			is += v;
		}
...
void ns_analyzed_image_time_path_element::context_coordinates_in_path_aligned_image(ns_vector_2i & pa_position, ns_vector_2i & pa_size, ns_vector_2i & context_tl, const ns_image_properties& path_aligned_image_size) const {
	ns_vector_2i shift(intensity_center_of_mass);
	const ns_vector_2i center(path_aligned_image_size.width/2, path_aligned_image_size.height/2);

	if (abs(shift.x) > ns_analyzed_image_time_path::maximum_pa_shift_offset()+ns_analyzed_image_time_path::maximum_pa_crop()) {
		std::cerr << "Completely Saturated during centering to intensity center: x=" << shift.x << "\n";
		pa_position.x = center.x - ns_analyzed_image_time_path::maximum_pa_shift_offset();
		pa_size.x = worm_context_size_.x - ns_analyzed_image_time_path::maximum_pa_crop();
		...
		context_tl.x = overflow;
	}
	else {
		pa_position.x = center.x - shift.x;
		pa_size.x = worm_context_size_.x;
		context_tl.x = 0;
	}

	if (abs(shift.y) > ns_analyzed_image_time_path::maximum_pa_shift_offset() + ns_analyzed_image_time_path::maximum_pa_crop()) {
		std::cerr << "Completely Saturated during centering to intensity center: y=" << shift.y << "\n";
		pa_position.y = center.y - ns_analyzed_image_time_path::maximum_pa_shift_offset();
		pa_size.y = worm_context_size_.y - ns_analyzed_image_time_path::maximum_pa_crop();
		...
		context_tl.y = overflow;
	}
	else {
		pa_position.y = center.y - shift.y;
		pa_size.y = worm_context_size_.y;
		context_tl.y = 0;
	}
}
...
bool ns_analyzed_image_time_path::populate_images_from_region_visualization(const unsigned long time,const ns_image_standard &region_visualization,const ns_image_standard & interpolated_region_visualization,bool just_do_a_consistancy_check, ns_analyzed_image_time_path::ns_load_type load_type, ns_time_path_image_movement_analysis_memory_pool<allocator_T> & memory_pool_, const long & debug_worm_id){
	...
	for (unsigned int k = 0; k < elements.size(); k++){
		...
		if (!just_flag_elements_as_loaded && !just_do_a_consistancy_check && was_previously_loaded) {
			ns_ex ex("Trying to re-load a previously loaded element for worm ");
			ex << debug_worm_id;
		...
		try{
			...
			if (e.inferred_animal_location) {
				if (interpolated_region_visualization.properties().width == 0)
					throw ns_ex("No interpolated region image has been specified from which to extract inferred worm image for worm ") << debug_worm_id;
				im = &interpolated_region_visualization;
				if (im->properties().width == 0)
					throw ns_ex("Required interpolated region image was not provided for worm ") << debug_worm_id;
			}
			else
				if (im->properties().width == 0)
					throw ns_ex("Required region image was not provided for worm ") << debug_worm_id;

			if (e.path_aligned_images == 0)
				throw ns_ex("Encountered unloaded image for worm ") << debug_worm_id;

			const ns_vector_2i center = ns_image_intensity_center_from_channel(*im, e.context_offset_in_region_visualization_image(), e.context_offset_in_region_visualization_image()+ e.worm_context_image_size(), 1);
			/*
			ns_image_standard im2;
			pp(e.worm_context_size().y, e.worm_context_size().x, 3);
			im2.init(pp);
			for (unsigned y = 0; y < pp.height; y++)
				for (unsigned x = 0; x < pp.width; x++) {
					im2[y][3 * x] = (*im)[e.context_offset_in_region_visualization_image().y+y][3 * (x+ e.context_offset_in_region_visualization_image().x) + 1];
					im2[y][3 * x+1] = (*im)[e.context_offset_in_region_visualization_image().y+y][3 * (x + e.context_offset_in_region_visualization_image().x) + 1];
					im2[y][3 * x + 2] = 255*(ns_vector_2i(x,y) == center);
				}
			...
			*/
			...
			if (!just_do_a_consistancy_check && !just_flag_elements_as_loaded) {
				//std::cerr << "INIT: " << k << " context size: " << e.worm_context_size() << "; cmass: " << e.intensity_center_of_mass << "pa_pos: " << tl_worm_context_position_in_pa << "\n";
			...
			//fill in center
			ns_vector_2i br_worm_context_position_in_pa(tl_worm_context_position_in_pa_ + context_pa_size);
			if (br_worm_context_position_in_pa.x >= path_aligned_image_image_properties.width || br_worm_context_position_in_pa.y >= path_aligned_image_image_properties.height)
				throw ns_ex("Context image overflows bounds of PA image for worm ") << debug_worm_id << " : PA size (" << path_aligned_image_image_properties.width << ","
				<< path_aligned_image_image_properties.height << "); context pos:(" << tl_worm_context_position_in_pa_.x << "," << tl_worm_context_position_in_pa_.y << ") "
				<< " for a worm with an image size (" << e.size_in_path_aligned_image.x << "," << e.size_in_path_aligned_image.y << ")";
			...
			for (int i = 0; i < animals[k].machine_state_info.path.size(); i++) {
				for (unsigned int j = 0; j < animals[k].machine_state_info.path[i].sub_measurements.size(); j++) {
					...
					if (!std::isfinite(animals[k].machine_state_info.path[i].sub_measurements[j]) || !std::isfinite(measurement_averages[j]))
						std::cerr << "Infinite measurement observed\n";
				}
			}
		...
		for (unsigned int i = 0; i < animals[k].machine_state_info.path.size(); i++) {
			...
			for (unsigned int pp = 0; pp < animals[0].state_info_variable_names.size(); pp++) {
				...
				o << "," << animals[k].machine_state_info.path[i].sub_measurements[pp]
				  ...
				  << "," << animals[k].by_hand_state_info.path[i].log_sub_probabilities[pp]
				  << "," << diff_p_sub
...
void ns_time_path_image_movement_analyzer<allocator_T>::output_visualization(const string & base_directory) const{
	...
	all_quant << "path_id,time,stationary,movement\n";
...
class ns_stabilized_movement_data_accessor {
public:
	ns_stabilized_movement_data_accessor(ns_analyzed_image_time_path::ns_element_list& l) :elements(&l) {}
	unsigned long size()const { return elements->size(); }
	const unsigned long time(const unsigned long i) { return (*elements)[i].absolute_time; }
	double raw(const unsigned long i) {
...
class ns_stabilized_scaled_movement_data_accessor {
public:
	ns_stabilized_scaled_movement_data_accessor(ns_analyzed_image_time_path::ns_element_list& l) :elements(&l) {}
	unsigned long size()const { return elements->size(); }
	const unsigned long time(const unsigned long i) { return (*elements)[i].absolute_time; }
	double raw(const unsigned long i) {
...
/*
class ns_stabilized_movement_data_min_accessor {
public:
	ns_stabilized_movement_data_min_accessor(ns_analyzed_image_time_path::ns_element_list& l) :elements(&l) {}
	unsigned long size()const { return elements->size(); }
	const unsigned long time(const unsigned long i) { return (*elements)[i].absolute_time; }
	double raw(const unsigned long i) {
		return (*elements)[i].measurements.spatial_averaged_movement_score_uncropped;
	}
	double& processed(const unsigned long i) {
		return (*elements)[i].measurements.spatial_averaged_movement_score_uncropped_min_4x;
	}
private:
	ns_analyzed_image_time_path::ns_element_list* elements;
};*/
...
public:
	...
	void operator()(const int kernel_width, T& data) {
		...
		for (long i = 0; i < data.size(); i++) {
			...
			if (smoothing_type == ns_mean) {
			...
			else if (smoothing_type == ns_median){
				...
				c_min = data.raw(start);
				for (unsigned int j = start + 1; j < stop; j++)
					if (c_min > data.raw(j))
						c_min = data.raw(j);
				data.processed(i) = c_min;
			}
		}
	}
private:
...
void ns_analyzed_image_time_path::denoise_movement_series_and_calculate_intensity_slopes(const ns_time_series_denoising_parameters & times_series_denoising_parameters, std::vector<ns_64_bit >& tmp1, std::vector<ns_64_bit >& tmp2){
...

	{
		...
		smooth_data(v1_movement_kernel_width, acc);
		...
		smooth_data_4(v2_movement_kernel_width, acc_4);

		ns_stabilized_scaled_movement_data_accessor acc_5(elements);
		ns_kernel_smoother < ns_stabilized_scaled_movement_data_accessor, ns_mean> smooth_data_5;
...
void ns_match_histograms(const ns_histogram<unsigned int, ns_8_bit> h1, const ns_histogram<unsigned int, ns_8_bit> h2,float * histogram_matching_factors) {
	double quartiles_1[3], quartiles_2[3];
	for (unsigned int i = 1; i < 4; i++)
		quartiles_1[i-1] = h1.average_of_ntile(i, 4, false);
	for (unsigned int i = 1; i < 4; i++)
		quartiles_2[i-1] = h1.average_of_ntile(i, 4, false);
	double mean_fold_change(0);
	for (unsigned int i = 0; i < 3; i++) {
		if (quartiles_1[i] == 0)
			histogram_matching_factors[i] = 0;
		else
			histogram_matching_factors[i] = quartiles_2[i] / quartiles_1[i];
		mean_fold_change += histogram_matching_factors[i];
	}
...
void ns_analyzed_image_time_path::quantify_movement(const ns_analyzed_time_image_chunk & chunk){
	...
	for (unsigned int i = chunk.start_i; i < chunk.stop_i; i++) {
		...
		elements[i].measurements.intensity_normalization_scale_factor = elements[i].histogram_matching_data.overall_scaling;
		...
		for (long y = 0; y < elements[i].registered_images->movement_image_.properties().height; y++) {
			for (long x = 0; x < elements[i].registered_images->movement_image_.properties().width; x++) {
				...
				if (worm_threshold) {
					...
					ns_8_bit cur_intensity = elements[i].registered_images->image[y][x];
...
void ns_analyzed_image_time_path::copy_aligned_path_to_registered_image(const ns_analyzed_time_image_chunk & chunk, std::vector < ns_image_standard> & temporary_images, ns_time_path_image_movement_analysis_memory_pool<allocator_T> & memory_pool_){
	...
	for (long i = chunk.start_i; ; i += istep) {
		...
		if (1) {
			...
			ns_vector_2i br(tl + elements[i].size_in_path_aligned_image);
			...
			ns_vector_2i br_registered(tl_registered + elements[i].size_in_path_aligned_image);
...
struct ns_image_quant_info {
	ns_image_quant_info() {}
	ns_image_quant_info(int source, int dest, double d_) :source_id(source), d(d_) {}
	int source_id;
	double d;
};
bool operator<(const ns_image_quant_info& a, const ns_image_quant_info& b) {
	return a.d < b.d;
}

double ns_calc_whole_image_scaling(const ns_image_standard & subject, const ns_image_standard & reference) {
	const ns_64_bit min_pixel_value_for_scaling_text(40);
	double d = 0;
	long count(0);
	for (unsigned int y = 0; y < subject.properties().height; y++)
		for (unsigned int x = 0; x < subject.properties().width; x++) {
			if (subject[y][x] >= min_pixel_value_for_scaling_text&& reference[y][x] >= min_pixel_value_for_scaling_text) {
				d += reference[y][x] / (double)subject[y][x];
				count++;
			}
		}
	if (count < 50) 
...
ns_image_quant_info ns_calc_mean(const ns_image_standard& subject) {
	ns_image_quant_info r;
	ns_64_bit m = 0;
	long count(0);
	for (unsigned int y = 0; y < subject.properties().height; y++)
		for (unsigned int x = 0; x < subject.properties().width; x++) {
...
void ns_analyzed_image_time_path::calculate_movement_images(const ns_analyzed_time_image_chunk & chunk, const ns_analyzed_time_image_chunk& first_chunk) {
	...
	for (long i = chunk.start_i; ; i += istep) {
		...
		//note that the movement image pixels are scaled, but the registered image pixels are unscaled (so we can inspect the original if needed)
		...
		if (intensity_normalization_kernal_stop >= upper_kernel_boundary)
			intensity_normalization_kernal_stop = upper_kernel_boundary;
		if (intensity_normalization_kernal_start < lower_kernel_boundary)
			intensity_normalization_kernal_start = lower_kernel_boundary;
		//std::vector< const ns_histogram_matching_data *> hist_data(intensity_normalization_kernal_stop - intensity_normalization_kernal_start + 1);
		const int num_images = intensity_normalization_kernal_stop - intensity_normalization_kernal_start + 1;
		...
		if (num_images >= 3) {
			...
			//find the image with the median mean intensity, and use it as the reference.
			std::nth_element(means.begin(), means.begin()+ intensity_normalization_kernel_half_width,means.end());
			const unsigned long ref_id = means[intensity_normalization_kernel_half_width].source_id;
			cur_scaling = ns_calc_whole_image_scaling(elements[i].registered_images->image, elements[ref_id].registered_images->image);
		}
		else {
			cur_scaling = 1;
		}	
		...
		if (exp(abs(log(cur_scaling))) > 3)
			cur_scaling = 1;
		if (exp(abs(log(movement_dt_scaling))) > 3)
			movement_dt_scaling= 1;
		...
		//const ns_vector_2d v1(first_frames ? ns_vector_2d(0, 0) : elements[i - movement_dt].registration_offset);
...
template<class allocator_T>
void ns_time_path_image_movement_analyzer<allocator_T>::match_plate_areas_to_paths(std::vector<ns_region_area>& areas) {
...
ns_analyzed_time_image_chunk ns_analyzed_image_time_path::initiate_image_registration(const ns_analyzed_time_image_chunk & chunk,ns_alignment_state & state, ns_calc_best_alignment_fast & align){
	...
	if (first_frame) {
		...
		state.element_occupancy_ids.clear();
	}
	...
	if (first_frame) {
		const ns_vector_2i tl_worm_context_position_in_pa_ = elements[first_index].offset_in_path_aligned_image,
		...
		state.element_occupancy_ids.push_back(first_index);
	...
	for (long i = remaining.start_i; i != remaining.stop_i; i+=step){
		...
		const ns_vector_2i tl_worm_context_position_in_pa_ = elements[i].offset_in_path_aligned_image,
			context_pa_size = elements[i].size_in_path_aligned_image;
		//ns_vector_2i tl_worm_context_position_in_pa_, context_pa_size, context_internal_tl;
		//elements[first_index].context_coordinates_in_path_aligned_image(tl_worm_context_position_in_pa_, context_pa_size, context_internal_tl, prop);
		const ns_vector_2i br_worm_context_position_in_pa(tl_worm_context_position_in_pa_ + context_pa_size);
...
void ns_analyzed_image_time_path::calculate_image_registration(const ns_analyzed_time_image_chunk & chunk_, ns_alignment_state & state, const ns_analyzed_time_image_chunk & first_chunk, ns_calc_best_alignment_fast & align) {
	...
	for (long i = chunk.start_i; ; i += step) {
		...
		//ns_vector_2i tl_worm_context_position_in_pa_, context_pa_size, context_internal_tl;
		//elements[i].context_coordinates_in_path_aligned_image(tl_worm_context_position_in_pa_, context_pa_size, context_internal_tl, prop);
		const ns_vector_2i tl_worm_context_position_in_pa_ = elements[i].offset_in_path_aligned_image,
							context_pa_size = elements[i].size_in_path_aligned_image;
		const ns_vector_2i br_worm_context_position_in_pa(tl_worm_context_position_in_pa_ + context_pa_size);

		//debug code that can add synthetic shift to a random amount up and left
		//so we can then test the registration algorithm's ability to fix it
	...
	#endif
	#ifdef NS_USE_FAST_IMAGE_REGISTRATION
	std::string dbg(ns_to_string(i));
		t.start();
		elements[i].registration_offset =
			align(state.registration_offset_average(),	//path_aligned_images are loaded with their intensity center of mass at the center, and so is the consensus, so the expected alignment is (0,0)
				maximum_alignment_offset(),
				state, elements[i].path_aligned_images->image,
				elements[i].saturated_offset, tl_worm_context_position_in_pa_, context_pa_size, false, dbg);
		...
		/*if (state.debug_out == 0)
			state.debug_out = new ofstream("c:\\server\\debug_out.txt");
		*state.debug_out << i << " reg: " << elements[i].registration_offset << (elements[i].saturated_offset ? " SAT" : "") << " " << " sz: " << elements[i].worm_context_size() << " CM: " << elements[i].intensity_center_of_mass << " " ;
		*state.debug_out <<" pa_pos: " << tl_worm_context_position_in_pa << " : " << br_worm_context_position_in_pa << " sh: " << state.cumulative_recentering_shift << "\n";
		cout << i << " reg: " << elements[i].registration_offset << (elements[i].saturated_offset ? " SAT" : "") << " " << " sz: " << elements[i].worm_context_size() << " CM: " << elements[i].intensity_center_of_mass << " ";
		cout <<  " pa_pos: " << tl_worm_context_position_in_pa << " : " << br_worm_context_position_in_pa << " sh: " << state.cumulative_recentering_shift << "\n";
		*/
		...
		for (unsigned int y = 0; y < im.properties().height; y++)
			for (unsigned int x = 0; x < im.properties().width; x++) {
				im[y][3 * x] = (state.consensus_count[y][x] != 0) ? state.consensus[y][x] / state.consensus_count[y][x] : 0;
		...
		if (tl_worm_context_position_in_pa_.y - elements[i].registration_offset.y < 0 ||
			...
			if (tl2.y < 0) tl2.y = 0;
			if (tl2.x < 0) tl2.x = 0;
			if (br2.y > prop.height) br2.y = prop.height;
			if (br2.x > prop.width) br2.x = prop.width;
		...
		const ns_vector_2i d( ns_vector_2i(prop.width, prop.height)/2- ns_image_intensity_center(state.consensus,state.consensus_count,ns_vector_2i(0, 0), ns_vector_2i(prop.width, prop.height)));
...
struct ns_index_orderer{
	ns_index_orderer(const ns_time_element_link * path_id,const unsigned long d): stationary_path_id(path_id), time(d){}

	bool operator<(const ns_index_orderer & r) const {return time < r.time;}

	const ns_time_element_link* stationary_path_id;
	unsigned long time;
};
...
ns_analyzed_image_time_path_group<allocator_T>::ns_analyzed_image_time_path_group(const ns_stationary_path_id group_id, const ns_64_bit region_info_id,const ns_time_path_solution & solution_, const ns_death_time_annotation_time_interval & observation_time_interval,ns_death_time_annotation_set & rejected_annotations,ns_time_path_image_movement_analysis_memory_pool<allocator_T> & memory_pool){
	...
	for (unsigned int i = 0; i < solution_.path_groups[group_id.group_id].path_ids.size(); i++){
		...
		//first we sort all solution elements by time, to identify any duplicates
		vector<ns_index_orderer> path_elements_sorted_by_time;
		path_elements_sorted_by_time.reserve(source_path.stationary_elements.size());
		...
		const unsigned long first_time_index((*path_elements_sorted_by_time.begin()).stationary_path_id->t_id);
		...
		for (unsigned int j = 0; j < path_elements_sorted_by_time.size(); j++){
			...
			if (s > 0 && path.elements[s-1].absolute_time == absolute_time){
				...
				std::cerr << "Skipping frames at frame " << s << "\n";
			...
			path.elements[s].context_position_in_region_vis_image = source_element.context_image_position_in_region_vis_image;
			path.elements[s].inferred_animal_location = source_element.inferred_animal_location;
			path.elements[s].element_before_fast_movement_cessation = source_element.element_before_fast_movement_cessation;

			path.elements[s].worm_region_size_ = source_element.region_size;
			path.elements[s].worm_context_size_ = source_element.context_image_size;

			path.elements[s].part_of_a_multiple_worm_disambiguation_group = source_element.part_of_a_multiple_worm_disambiguation_cluster;
			...
			path.elements[s].region_position_in_source_image = source_element.region_position;
			path.elements[s].context_position_in_source_image = source_element.context_image_position;
			path.elements[s].subregion_info = source_element.subregion_info;
		}
		...
		//the center of mass is allowed to be offset from the actual image center by ns_analyzed_image_time_path::maximum_pa_shift_offset() in either direction
		{
			ns_vector_2i br(0,0);
			for (unsigned int j = 0; j < path.elements.size(); j++) {
				const ns_analyzed_image_time_path_element& e(path.elements[j]);
			...
			//the size of the path image required to contain all centered context images
...
void ns_analyzed_image_time_path::save_movement_images(const ns_analyzed_time_image_chunk & chunk, ns_sql & sql, const ns_images_to_save & images_to_save, const ns_backwards_image_handling & backwards_image_handling, const ns_output_location & output_location){
	...
	if (chunk.start_i == chunk.stop_i)
		throw ns_ex("Empty chunk requested!");
	#ifndef NS_CALCULATE_OPTICAL_FLOW
	if (save_flow_image)
...
void ns_analyzed_image_time_path::save_movement_image(const ns_analyzed_time_image_chunk & chunk, ns_image_storage_reciever_handle<ns_8_bit> & out, const bool only_write_backwards_frames) {
	...
	std::set<unsigned int> elements_written;
		...
		for (unsigned long dy = 0; dy < dh; dy++) {
			...
			for (unsigned long i = 0; i < d.grid_dimensions.x; i++) {
				...
				if (i_offset + i >= elements.size()) {
					...
					elements_written.emplace(i + i_offset);
	...
	for (auto p = elements_written.begin(); p != elements_written.end(); ++p){
		...
		if (e.debug_write_count > 1)
			cout << "Multiple writes: " << e.debug_write_count << "\n";
	}
...
void ns_analyzed_image_time_path_element_measurements::zero(){
	...
	intensity_normalization_scale_factor = 0;
...
void ns_analyzed_image_time_path_element_measurements::rescale_baseline_intensities(double scale_factor) {
	total_intensity_within_region = total_intensity_within_region * scale_factor;
	total_intensity_within_stabilized = total_intensity_within_stabilized * scale_factor;
	total_intensity_within_stabilized_denoised = total_intensity_within_stabilized_denoised * scale_factor;
	total_intensity_within_foreground = total_intensity_within_foreground * scale_factor;
	total_intensity_within_alternate_worm = total_intensity_within_alternate_worm * scale_factor;
}
void ns_analyzed_image_time_path_element_measurements::square() {
	...
	intensity_normalization_scale_factor = intensity_normalization_scale_factor * intensity_normalization_scale_factor;
...
void ns_analyzed_image_time_path_element_measurements::square_root() {
	...
	intensity_normalization_scale_factor = sqrt(intensity_normalization_scale_factor);
...
ns_analyzed_image_time_path_element_measurements operator+(const ns_analyzed_image_time_path_element_measurements & a, const ns_analyzed_image_time_path_element_measurements & b){
	...
	ret.intensity_normalization_scale_factor = a.intensity_normalization_scale_factor +b.intensity_normalization_scale_factor;
...
ns_analyzed_image_time_path_element_measurements operator/(const ns_analyzed_image_time_path_element_measurements & a, const int & d) {
	...
	ret.intensity_normalization_scale_factor = a.intensity_normalization_scale_factor / d;
...
ns_ostream* ns_get_histogram_output(ns_64_bit region_info_id, int hist_size, ns_sql & sql) {
	...
	ns_image_server_results_file hist_out(image_server.results_storage.time_path_image_analysis_quantification(sub, "histograms", true, sql, false, true, ns_csv_gz));
		...
		for (unsigned int i = 0; i < groups.size(); i++){
			for (unsigned int j = 0; j < groups[i].paths.size(); j++){
				...
				description_series.timepoints[t].worms.push_back(
					...
					groups[i].paths[j].elements[tt].worm_context_image_size(),
