...
//registered images are the final output of image registration,
//generated by shifting path_aligned images so that they line up
//registered images are the size of path, which is determined 
//by the function set_path_alignment_image_dimensions and is the 
//size of the path (which is is the bounding box around all region images in that path)
//increased to include the maximum image registration offset of any image.
//when images are shifted, any "empty" exposed are fillled in as black
...
public:
	...
	ns_vector_2d worm_center_in_registered_image() const;
	ns_vector_2d worm_center_in_absolute_coordinates() const;
	mutable ns_vector_2d worm_center_in_registered_image_cached;
	mutable bool worm_center_in_registered_image_cached_calculated;
	...
	const ns_vector_2d & registration_offset_v()const { return registration_offset; }
...
public:
	...
	//the maximum distance the alignment algorithm can shift a worm to match the previous frame
	//(note that all frames are centered around their image intensity center of mass before registration
...
public:
	...
	void reanalyze_with_different_movement_estimator(const ns_time_series_denoising_parameters &,const double vigorous_movement_threshold,const ns_analyzed_image_time_path_death_time_estimator * e);
	...
	void match_plate_areas_to_paths(std::vector<ns_region_area> & areas);
...
private:
	...
	void load_region_visualization_images(const unsigned long start_i, const unsigned long stop_i,const unsigned int start_group, const unsigned int stop_group,ns_sql & sql, bool just_do_a_consistancy_check,bool running_backwards, ns_analyzed_image_time_path::ns_load_type just_flag_elements_as_loaded, const long & specific_worm=-1);
	void acquire_region_image_specifications(const ns_64_bit region_id,ns_sql & sql);
	void get_output_image_storage_locations(const ns_64_bit region_id, const ns_64_bit& specific_worm_id,ns_sql & sql,const bool create_only_flow);
