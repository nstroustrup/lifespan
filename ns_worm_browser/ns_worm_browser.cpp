...
void ns_worm_learner::output_distributions_of_detected_objects(const std::string &directory){
	if (worm_detection_results == 0)
	 ...
	 const std::vector<ns_detected_worm_info *> &worms = worm_detection_results->actual_worm_list();

	 const std::vector<ns_detected_worm_info *> &non_worms=worm_detection_results->non_worm_list();
