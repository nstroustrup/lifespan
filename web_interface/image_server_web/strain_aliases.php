...
if (ns_param_spec($_POST,'edit') && $_POST['edit'] =='1'){
  ...
  foreach ($strain_info as $id => $d){
    ...
    $query .= "SET strain='" . $sql->escape_string($d[0]) . "', genotype='".$sql->escape_string($d[1])."', conditions='".$sql->escape_string($d[2])."'";
   ...
   if (FALSE && $update_experiment){
	 ...
	 $query = "UPDATE sample_region_image_info as r, capture_samples as s "
	   . "SET r.strain = '" . $sql->escape_string($d[0]) .
	   "' WHERE r.strain='" . $sql->escape_string($known_strains_by_id[$id])
